...
public class TokenDepositServiceImpl implements TokenDepositService {
    ...
    public String depositToProduct(String userEmail, Long subscribeId, BigDecimal amount) {
        ...
        switch (product.getType()) {
            case VOUCHER -> handleVoucher(account, product, amount); // 상품권
            case SAVING -> handleSaving(account, product, amount); // 예금
            case INSTALLMENT -> handleInstallment(account, product, amount); // 적금
        }
        return "예치 완료";
    }
    ...
    private void handleVoucher(Account account, Product product, BigDecimal amount) {
        ...
        Subscribe subscribe = subscribeRepository
                .findByuser_EmailAndProductId(account.getActor().getEmail(), product.getId())
                .orElseThrow(() -> new RuntimeException("가입된 상품권이 없습니다"));

        // 2. 해당 상품이 속한 뱅크 토큰 조회
        BankToken productBankToken = product.getBankToken();

        // 3. 사용자 계정 중 bankToken 계좌 조회
        Account bankTokenAccount = accountRepository
                .findByActor_EmailAndBankTokenId(account.getActor().getEmail(), productBankToken.getId())
                .orElseThrow(() -> new RuntimeException("사용자의 은행 토큰 계좌가 없습니다."));


        // 4. 월 최대 한도 체크 (월 최대 한도 초과시 예외 처리)
    ...
    // 예금 예치
    private void handleSaving(Account account, Product product, BigDecimal amount) {
        // 1. 가입 정보 조회
        Subscribe subscribe = subscribeRepository
                .findByuser_EmailAndProductId(account.getActor().getEmail(), product.getId())
                .orElseThrow(() -> new RuntimeException("가입된 예금 상품이 없습니다."));
        ...
        // 3. 해당 상품이 속한 뱅크 토큰 조회
        BankToken productBankToken = product.getBankToken();

        // 4. 사용자 계정 중 bankToken 계좌 조회
        Account bankTokenAccount = accountRepository
                .findByActor_EmailAndBankTokenId(account.getActor().getEmail(), productBankToken.getId())
                .orElseThrow(() -> new RuntimeException("사용자의 은행 토큰 계좌가 없습니다."));
    ...
    // 적금 예치
    private void handleInstallment(Account account, Product product, BigDecimal amount) {
        // 1. 가입 정보 조회
        Subscribe subscribe = subscribeRepository
                .findByuser_EmailAndProductId(account.getActor().getEmail(), product.getId())
                .orElseThrow(() -> new RuntimeException("가입된 적금 상품이 없습니다"));
